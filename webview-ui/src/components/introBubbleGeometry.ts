...
export function computeIntroBubbleGeometry(frame: IntroBubbleFrame): IntroBubbleGeometry {
  ...
  // into the container.
  const anchorX = project.toScreenX(greeter.x + CONSENT_BUBBLE_OFFSET_X_WORLD);
  const anchorY = project.toScreenY(greeter.y - CONSENT_BUBBLE_ANCHOR_RISE_WORLD);
  ...
  const offX = Math.min(
    (CONSENT_BUBBLE_OFFSET_X_WORLD + bubbleWorldW - CHARACTER_HIT_HALF_WIDTH) / 2,
    project.viewportWorldWidth * CONSENT_CAMERA_MAX_X_OFFSET_VIEWPORT_FRACTION,
  );
  const offY = Math.min(
    (CONSENT_BUBBLE_ANCHOR_RISE_WORLD + bubbleWorldH) / 2,
    project.viewportWorldHeight / 2 - CONSENT_CAMERA_MIN_CHAR_VISIBLE_WORLD,
  );
  ...
  const downShift = project.toWorldLength(CONSENT_CAMERA_DOWN_SHIFT_PX);
  const cameraTarget = {
    x: greeter.x + offX,
