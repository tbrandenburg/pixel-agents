...
- `e2e/claude/hooks-on/consent.spec.ts:136` — fresh install: the dialog discloses scope and Install writes the hooks (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:201` — Not Now writes nothing and leaves consent ungranted (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:226` — Don't Ask Again writes nothing and persists hooks off (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:250` — dismissing the dialog writes nothing, exactly like Not Now (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:282` — clicking the backdrop neither answers nor dismisses (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:317` — a pre-consent 14-event install migrates to 12 with no prompt (Hooks consent gate / pre-consent install)
- `e2e/claude/hooks-on/consent.spec.ts:357` — Settings toggle removes the migrated hooks and keeps third-party entries (Hooks consent gate / pre-consent install)
- `e2e/claude/hooks-on/consent.spec.ts:403` — a failed uninstall does not persist hooks-off (Hooks consent gate / toggle-off failure)
...
- `e2e/standalone/hooks.spec.ts:142` — the tokened page shows the consent dialog and Install writes the hooks (Standalone / hooks consent)
- `e2e/standalone/hooks.spec.ts:166` — an untokened spectator page never sees the consent dialog (Standalone / hooks consent)
- `e2e/standalone/hooks.spec.ts:195` — the hooks checkbox reflects install state and its click is the consent grant (Standalone / hooks consent)
