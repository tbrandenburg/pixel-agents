...
88 tests total. Generated by `scripts/generate-e2e-inventory.mjs`. Re-run after adding or removing tests.
...
- `e2e/claude/hooks-on/consent.spec.ts:150` — fresh install: the dialog discloses scope and Install writes the hooks (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:230` — Not Now writes nothing and leaves consent ungranted (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:255` — Don't Ask Again writes nothing and persists hooks off (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:279` — dismissing the dialog writes nothing, exactly like Not Now (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:311` — clicking the office around the bubble neither answers nor dismisses (Hooks consent gate)
- `e2e/claude/hooks-on/consent.spec.ts:348` — a pre-consent 14-event install migrates to 12 with no prompt (Hooks consent gate / pre-consent install)
- `e2e/claude/hooks-on/consent.spec.ts:388` — Settings toggle removes the migrated hooks and keeps third-party entries (Hooks consent gate / pre-consent install)
- `e2e/claude/hooks-on/consent.spec.ts:434` — a failed uninstall does not persist hooks-off (Hooks consent gate / toggle-off failure)
...
- `e2e/standalone/hooks.spec.ts:143` — the tokened page shows the consent dialog and Install writes the hooks (Standalone / hooks consent)
- `e2e/standalone/hooks.spec.ts:167` — an untokened spectator page never sees the consent dialog (Standalone / hooks consent)
- `e2e/standalone/hooks.spec.ts:196` — the hooks checkbox reflects install state and its click is the consent grant (Standalone / hooks consent)
