...
/**
 * The in-app first-run consent flow: the server asks over the wire
 * (hooksConsentRequest during the webviewReady handshake) and acts on the
 * answer (hooksConsentResponse). These translate the semantics the retired
 * TTY prompt pinned in cli.test.ts — most importantly that nothing but an
 * exact, explicit approval ever installs, and that junk is never read as one.
 */
describe('clientMessageHandler: hooks consent flow', () => {
  ...
  beforeEach(() => {
    tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'pxl-cmh-consent-'));
    originalHome = process.env.HOME;
    process.env.HOME = tempHome;
