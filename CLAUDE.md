...
server/                              Lifecycle runtime + Fastify HTTP/WS server
  src/
    providers/hook/claude/           Reference HookProvider — only place that knows Claude specifics
      ...
      consentGate.ts                 Shared first-run consent POLICY: when to ask (hooksConsentRequest) and what an answer means (consentActionFor(choice, {installed, consentGiven}) — state-aware so an Intro revision undoes whatever the earlier answer left, hooks on disk OR a grant a failed install recorded)
      consentExecutor.ts             Shared consent EXECUTION: applyConsentChoice(choice, ConsentEffects) runs the five actions in one order for both surfaces (only the effects differ) and SERIALIZES answers per process — revision is designed-for, and an unserialized one reads a mid-flight install as "nothing installed"
...
- **Consent before any FIRST `~/.claude/settings.json` write**, one shared `hooksConsentGiven` flag across surfaces. **Exactly one population is prompted: the one with nothing of ours installed.** With our hooks already present but no recorded consent (a pre-consent version installed them silently), consent is granted at startup and the install runs with **no prompt at all** — deliberate zero friction, since that install only ever removes events, never adds them. The accepted cost: consent is read off artifacts our own pre-consent code wrote, and afterwards the **only** removal route is the Settings toggle the disclosure names (pinned in `consent.spec.ts`). The branch is chosen by `areHooksInstalled` = **ANY of our commands on ANY event** — an all-or-nothing reading made a partial install look like "nothing installed", so live hooks were presented as absent and "Don't Ask Again" persisted hooks-off without removing them. **The ask is one step of the Intro** — the four-step first-run tour (`webview-ui/src/components/IntroBubble.tsx`): Welcome → Powered by Claude Code (install pointer: `CLAUDE_CODE_INSTALL_COMMAND` + `CLAUDE_CODE_URL`) → hooks consent → You're all set (Discord invite). IN-APP and identical on both surfaces: no native VS Code modal, no CLI TTY prompt. The server sends `hooksConsentRequest` during the `webviewReady` handshake (VS Code: `PixelAgentsViewProvider`; standalone: `clientMessageHandler`, **privileged connections only** — an untokened spectator's answer would be ignored, so it is never shown the tour), carrying `CONSENT_INSTALL_HEADLINE` + `CONSENT_DISCLOSURE` from `consentCopy.ts`; the consent step renders that payload verbatim with no client-side copy to drift, while the other three steps' copy is the webview's own and carries NO terms. Buttons: Back is `ghost`, Continue/Not Now `default`, Install Hooks and Let's Go `accent`, plus a top-right close `x` (`aria-label="Close"`, same ghost icon button as the modals) on every step; step dots mark progress. **The ask-or-not predicate, the choice→action rule, AND the execution live ONCE**: `consentGate.ts` (`hooksConsentRequest` / `consentActionFor(choice, {installed, consentGiven})`) decides, `consentExecutor.ts` (`applyConsentChoice(choice, ConsentEffects)`) performs — both surfaces supply only their own effects (error modal vs console, workspace setting vs store adapter). Centralizing the policy alone left the five-arm switch duplicated one layer down, where the same drift could return with the same consequence. The executor also **serializes** answers per process: Back-then-revise is designed-for, both surfaces dispatch without awaiting, and a revision that observes the first answer's install mid-flight reads `installed:false` and degrades to a no-op, leaving hooks installed against the user's final answer. **A consent-step click sends `hooksConsentResponse` immediately and the tour advances to its closing step**; the tour's wire-facing state machine (`webview-ui/src/hooks/introTourState.ts`, a pure reducer driven by `useIntroTour` and unit-tested in `introTour.test.ts`) snapshots the request and, once a choice was sent from this tour, ignores `consentRequest` going null — so the closing step survives the `hooksStatus installed:true` its own install broadcasts, while an UNANSWERED tour still closes when a cross-window install moots the ask. **Back from the closing step re-opens the consent step for a real change of mind, so a choice is an absolute state command** (see `docs/adr/0001`): `install` grants + installs (the same path as the Settings toggle — VS Code routes it through `setHooksEnabled(true)`), idempotent on repeat; `never` persists hooks-off, and when ours are on disk it takes the **full toggle-off path first** (`disable`: uninstall, persist only after the disk agrees); `notNow` writes nothing, and over anything an earlier answer LEFT it **reverts** (uninstall when ours are on disk, `revokeHooksConsent()` either way, preference untouched — the ask must genuinely return). "What an earlier answer left" is deliberately broader than "installed": `install` grants BEFORE it writes, so a failed install (the unparseable settings.json of issue #377 — exactly the population this gate exists for) leaves a grant with nothing on disk, and the grant ALONE retires the ask. Keying the revert on `installed` read that as nothing-to-undo and stranded the user with an ask that could never come back; revoking a grant with nothing installed touches no user file at all. Every unrecognized value writes nothing, and an unreadable settings file degrades to installed=false so no choice ever uninstalls on a guess. **The tour is DIEGETIC**: a greeter character (char_0, reserved id `GREETER_ID`) spawns near the office's bottom-left corner (target tile `GREETER_TILE_MARGIN` = 3 in from the left and bottom edges; nearest walkable tile when that is a seat/furniture/wall/VOID) and the `IntroBubble` is its speech bubble (anchored up-right of its head, `role="dialog"`; all geometry — clamped anchor, tail squares, camera target — is `introBubbleGeometry.ts`, pure and unit-tested). The greeter is not an agent, **structurally**: it lives in `OfficeState.greeter`, OUTSIDE the `characters` map, and joins the agents at exactly one seam (`getCharacters()`, the render feed) — so seat assignment, palette diversity, the wander FSM, hit-testing and the persisted seat payload (`getPersistableSeats`) exclude it with no per-consumer `isGreeter` guard to forget (the flag remains only as a snapshot marker for e2e). It lives exactly as long as the bubble is mounted; every close path despawns it via the matrix effect. The camera drifts to center character + bubble together (`OfficeState.greeterCameraTarget`, fed per-frame by the bubble overlay; an explicit `cameraFollowId` outranks it, and a manual pan/wheel cancels it via `cancelGreeterCamera` until the next spawn). **The close x and Escape abort the tour and send no message at all** — the whole Intro simply returns on the next open (an abort must never read as a durable decline), and clicks on the live office around the bubble are deliberately inert (a stray click is not an answer). **The closing step reports the OUTCOME, not the click**: an install can fail on a file we refuse to rewrite, and congratulating regardless leaves the user believing hooks are running over a file that was never written — so `useIntroTour` settles the verdict (the `installFailed` prop) on the ARRIVAL of a `hooksStatus` (`hooksStatusSeq`, since a failed install re-reports the same `false` the webview already held) and the step reads "Hooks couldn't be installed". Hooks are therefore not installed until the office is first opened by someone who can approve it. E2E: `launch.ts` and `helpers/standalone.ts` seed `hooksConsentGiven: true` as the baseline so the tour never covers the office in unrelated specs; the consent specs (`hooks-on/consent.spec.ts`, `standalone/hooks.spec.ts`) opt out and drive the tour through `frame.getByRole('dialog')` with the step-walking helpers in `e2e/helpers/intro.ts`.
