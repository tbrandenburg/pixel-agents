...
server/                              Lifecycle runtime + Fastify HTTP/WS server
  src/
    providers/hook/claude/           Reference HookProvider — only place that knows Claude specifics
      ...
      consentGate.ts                 Shared first-run consent POLICY: when to ask (hooksConsentRequest) and what an answer means (consentActionFor(choice, installed) — state-aware so an Intro revision over a landed install undoes it) — one implementation for both surfaces
