...
server/                              Lifecycle runtime + Fastify HTTP/WS server
  src/
    providers/hook/claude/           Reference HookProvider — only place that knows Claude specifics
      ...
      consentGate.ts                 Shared first-run consent POLICY: when to ask (hooksConsentRequest) and what an answer means (consentActionFor) — one implementation for both surfaces
...
- **Consent before any FIRST `~/.claude/settings.json` write**, one shared `hooksConsentGiven` flag across surfaces. **Exactly one population is prompted: the one with nothing of ours installed.** With our hooks already present but no recorded consent (a pre-consent version installed them silently), consent is granted at startup and the install runs with **no prompt at all** — deliberate zero friction, since that install only ever removes events, never adds them. The accepted cost: consent is read off artifacts our own pre-consent code wrote, and afterwards the **only** removal route is the Settings toggle the disclosure names (pinned in `consent.spec.ts`). The branch is chosen by `areHooksInstalled` = **ANY of our commands on ANY event** — an all-or-nothing reading made a partial install look like "nothing installed", so live hooks were presented as absent and "Don't Ask Again" persisted hooks-off without removing them. **The ask itself is IN-APP and identical on both surfaces**: no native VS Code modal, no CLI TTY prompt. The server sends `hooksConsentRequest` during the `webviewReady` handshake (VS Code: `PixelAgentsViewProvider`; standalone: `clientMessageHandler`, **privileged connections only** — an untokened spectator's answer would be ignored, so it is never shown the dialog), carrying `CONSENT_INSTALL_HEADLINE` + `CONSENT_DISCLOSURE` from `consentCopy.ts` in the payload so the webview renders the server's exact terms with no client-side copy to drift. **Both the ask-or-not predicate and the choice→action rule live ONCE, in `consentGate.ts`** (`hooksConsentRequest` / `consentActionFor`) — both surfaces call them; the copies each surface used to carry had already diverged (VS Code's Install skipped the persist-after-settle path, so a cross-window "Don't Ask Again" could leave hooks installed with the preference off). **The ask is DIEGETIC**: a greeter character (char_0, reserved id `CONSENT_GREETER_ID`) spawns near the office's bottom-left corner (target tile `CONSENT_GREETER_TILE_MARGIN` = 3 in from the left and bottom edges; nearest walkable tile when that is a seat/furniture/wall/VOID) and the webview's `ConsentBubble` is its speech bubble (anchored up-right of its head, `role="dialog"`, the three buttons; all geometry — clamped anchor, tail squares, camera target — is `consentBubbleGeometry.ts`, pure and unit-tested). The greeter is not an agent, **structurally**: it lives in `OfficeState.greeter`, OUTSIDE the `characters` map, and joins the agents at exactly one seam (`getCharacters()`, the render feed) — so seat assignment, palette diversity, the wander FSM, hit-testing and the persisted seat payload (`getPersistableSeats`) exclude it with no per-consumer `isGreeter` guard to forget (the flag remains only as a snapshot marker for e2e). It lives exactly as long as the bubble is mounted: any close path (a button, Escape, a `hooksStatus installed:true` mooting the ask) unmounts the bubble, which despawns the greeter via the matrix effect. The camera drifts to center character + bubble together (`OfficeState.consentCameraTarget`, fed per-frame by the bubble overlay; an explicit `cameraFollowId` outranks it, and a manual pan/wheel cancels it via `cancelConsentCamera` until the next spawn). The client answers with `hooksConsentResponse { choice: 'install' | 'notNow' | 'never' }`, handled **fail-closed on exact matches**: only `install` grants consent and installs (the same path as the Settings toggle — VS Code routes it through `setHooksEnabled(true)`), only `never` persists hooks-off (WITHOUT touching settings.json — nothing of ours is installed when the dialog shows), and `notNow` plus **every unrecognized value** writes nothing — a malformed choice must never be read as approval, or as a durable decline. Dismissal (Escape) sends **no message at all** and the request simply fires again on the next open; the bubble has no close `x`, no Cancel synonym, and clicks on the live office around it are deliberately inert (a stray click is not an answer). Hooks are therefore not installed until the office is first opened by someone who can approve it. E2E: `launch.ts` and `helpers/standalone.ts` seed `hooksConsentGiven: true` as the baseline so the dialog never covers the office in unrelated specs; the consent specs (`hooks-on/consent.spec.ts`, `standalone/hooks.spec.ts`) opt out and drive the dialog through `frame.getByRole('dialog')`.
